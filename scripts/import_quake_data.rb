...
class QuakeDataImporter
  ...
  def self.import
    ...
    games.each do |game|
      ...
      game[:kills].each do |kill|
        killer_id = find_player_id_by_name(conn, kill[:killer])
        victim_id = find_player_id_by_name(conn, kill[:victim])
        method = kill[:method]
  ...
  def self.find_player_id_by_name(conn, name)
